...
public class UserController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid input or user creation error",
                    content = @Content(mediaType = "application/json")),
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> updateUser(@Parameter(description = "ID of the user to update", required = true)
                                        @PathVariable Long id,
                                        @Parameter(description = "Updated user details", required = true)
                                        @Valid @RequestBody AdminSignupRequest updatedUser) {
        try {
